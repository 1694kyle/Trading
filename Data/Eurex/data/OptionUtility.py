...
class EurexWebPage(object):
    ...
    def get_available_dates(self):
        html_data = requests.get(self.url) #urllib.urlopen(self.url).read()
        webpage = html.fromstring(html_data.content)
    ...
    def get_option_series_data(self, is_call,
        ...
        target_url = (self.url +
                      self.param_url) % (option_type,
                                         ...
                                         option_dt.year,
                                         selected_date)
        html_data = requests.get(target_url) #urllib.urlopen(target_url).read()
        webpage = html.fromstring(html_data.content)
...
if __name__ == '__main__':
    vstoxx_subindex = VSTOXXSubIndex("vstoxx_sub_indexes.csv")
    # vstoxx_subindex.start(2)

    vstoxx_sub_indexes = pd.read_csv('vstoxx_sub_indexes.csv',
                                 index_col=[0],
                                 parse_dates=True, dayfirst=False)
    vstoxx = pd.read_csv('vstoxx.txt', index_col=[0],
                     parse_dates=True, dayfirst=False)
